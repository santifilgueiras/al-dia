...
// Validación de forma de lo que devuelve el modelo para fichas/simulacro/
// test de Explícamelo. extraerJSON ya cubre el parseo, pero nada verificaba
// que "correcta" esté entre 0 y 3, que haya 4 opciones reales o que no haya
// preguntas repetidas -- una ficha malformada entraba a fichas_cache y
// quedaba servida a TODOS los estudiantes para siempre (una opción múltiple
// con "correcta" fuera de rango es literalmente irresolvible). Devuelve
// null si la tanda vino mal de verdad (para que el endpoint pida
// reintentar en vez de servir/cachear algo cojo).
function validarFichas(fichas, esMultiple) {
  if (!Array.isArray(fichas) || !fichas.length) return null;
  const vistas = new Set();
  const limpias = [];
  for (const f of fichas) {
    if (!f || typeof f.pregunta !== 'string') continue;
    const pregunta = f.pregunta.trim();
    if (pregunta.length < 8) continue;
    const huella = pregunta.toLowerCase().replace(/[^a-záéíóúñ0-9]/g, '');
    if (vistas.has(huella)) continue; // duplicada
    if (esMultiple) {
      if (!Array.isArray(f.opciones) || f.opciones.length !== 4) continue;
      if (f.opciones.some(o => typeof o !== 'string' || !o.trim())) continue;
      const correcta = Number(f.correcta); // el modelo a veces manda "0" como string
      if (!Number.isInteger(correcta) || correcta < 0 || correcta > 3) continue;
      const opciones = f.opciones.map(o => o.trim());
      if (new Set(opciones.map(o => o.toLowerCase())).size !== 4) continue; // opciones repetidas
      vistas.add(huella);
      limpias.push({ ...f, pregunta, opciones, correcta });
    } else {
      if (typeof f.respuesta !== 'string' || f.respuesta.trim().length < 4) continue;
      vistas.add(huella);
      limpias.push({ ...f, pregunta, respuesta: f.respuesta.trim() });
    }
  }
  // Si se cayó más de un tercio, la respuesta estaba mal de verdad: mejor
  // reintentar que servir (y peor, cachear) una tanda coja.
  if (limpias.length < Math.max(4, Math.ceil(fichas.length * 0.66))) return null;
  return limpias;
}
